...
var reviews = [
    [null, '1', 'FIX ME', 1, '4', '5', '3', '4', '4', 'moderate', true, true, 'The flavours/tastes were brilliant. We tasted the trout, loved the scallops - very umami, only the duck was a bit problematic. It fell off the bone which is an indication it came from the freezer/cooler and was re-heated, but the taste was so good we ate the whole thing. The kitchen needs to dissolve the freezer issue. We will be back, but not for the duck.'],
    [null, '1', 'FIX ME', 2, '5', '5', '5', '5', '5', 'moderate', true, true, 'The food was simply amazing : from presentation to taste everything was spot on.\nUnlike the other “Indian” restaurants in the bay area, every curry tasted different!!'],
    [null, '1', 'FIX ME', 3, '3', '3', '3', '3', '2','moderate', false, true, 'not memorable. good enough for a lunch but not special in any way nor anything to go back to.'],
    [null, '1', 'FIX ME', 7, '3', '4', '1', '4', '3', 'moderate', false, true, 'Service was terrible, were seated without menus, asked for menus from the host after 20 min, sat for another 15 mean before we went back to the host and asked if we could have a waiter, the waiter finally arrived, i told him about my gluten allergy and he said he would get back to me about what dishes were GF and after waiting another 10 min when he never got back to us I asked the host (who was so kind) for another waiter, she brought the manager who was kind and confirmed my order was GF.'],
    [null, '1', 'FIX ME', 2, '5', '5', '5', '5', '5', 'moderate', true, true, 'One of my favorite places for brunch & sharing with out of town guest. Creative take on some classic Indian & American dishes.'],
    [null, '1', 'FIX ME', 9, '4', '4', '5', '5', '4', 'moderate', true, true, 'Disappointed - food is not as good as it was on previous visits (it’s been over a year since we were here). The only memorable dish was dessert (mango pana cotta). Delightful waitress and excellent cocktail.'],
    [null, '1', 'FIX ME', 3, '3', '3', '3', '4', '2', 'quiet', true, true, 'Bit overpriced for the food delivered'],
    [null, '1', 'FIX ME', 4, '3', '4', '3', '3', '1', 'loud', false, true, 'Food was ok, service was odd. Dishes placed on our table with comment like " this is chicken". We were pre theater and the could not get us out in 1 hour and 15 min. Pro- Drinks are awesome. Con- it\'s super expensive for fairly pedestrian food. We like Amber and Dosa much more.'],
    [null, '1', 'FIX ME', 1, '5', '5', '5', '5', '5', 'moderate', true, true, 'Fascinating Indian Fusion dishes! Wonderful, delicious and visually stunning mocktails and cocktails - I recommend the Pink City and the Spicy Gold drinks. We also enjoyed, for the meal, the very crispy baby spinach Palak Chaat, the mushroom and cheese Kulcha, and the Appams. Do check out their menus!! The decor is beautiful- an elegant interpretation very consistent with their style of cuisine: traditional elements of deep turquoise and peacock feather in a completely contemporary application. You must not miss this experience!'],
    [null, '1', 'FIX ME', 11, '4', '4', '5', '5', '4', 'moderate', true, true, 'Everything is great except the chicken kebab is very bland.']
];

var fullReviews = [];
for (let i = 0; i < REVIEW_COUNT; i++) {
    let review = reviews[i % reviews.length].slice();
    // compute which user authored the review
    review[2] = ((i % users.length) + 1).toString();
    // compute the date timestamp for MySQL
    let offset = review[3];
    let reviewDate = moment().subtract(offset, 'days').format();
...
connection.query('INSERT INTO restaurant_pages VALUE("1")', (err, results) => {
    if (err) {
        console.log('error while seeding restauran_pages');
    } else {
        console.log('seeding of restaurant_pages completed successfully');
    }
});
